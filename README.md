...
Чтобы точно определить историю выполнения и сократить количество возможных результатов, нам нужны модели согласованности (consistency models). Они описывают выполнение конкурентных потоков и определяют, в каком порядке выполнять операции и делать их результаты видимыми для других участников. 
...
1. ***Обработка на сервере происходит мгновенно***
2. ***Часы на удаленных компьютерах работают синхронно***
3. ***Состояние между узлами полностью согласовано***\
<q>В Apache Cassandra был найден баг, вызванный тем, что изменения схемы распространялись между репликами в разное время. Когда из БД пытались читать во время этого распространения, получалось так, что на одном сервере результаты кодировались, используя одну схему, а на другом сервере результаты декодировались, используя другую схему.</q>
4. ***Локальное и удаленное выполнение - это одно и то же***
5. ***Узлы всегда работают стабильно***

## Абстракции распределенных систем
Здесь автор начинает с описания канала, допускающего потерю сообщений и путем усиления гарантий доставки добирается до идеализированного представления канала, в котором потерь не существует. Для более наглядного представления я украду замечательную картинку из [статьи](https://medium.com/p/92ccad7d1403) Александра Поломодова. 

<img width="761" alt="Capture d’écran 2024-11-14 à 2 04 45 PM" src="https://github.com/user-attachments/assets/991e01dd-0079-4eb4-85de-10fb559a74d6">

## Задача двух генералов
Известная задача двух генералов представляет собой отличную аналогию достижения согласованности в распределенных системах. Заключается она примерно в следующем: 2 генерала собираются напасть на укрепленный город, для этого им необходимо договориться. Один из способов добиться этого изображен на рисунке ниже 

<img width="771" alt="Capture d’écran 2024-11-14 à 2 24 54 PM" src="https://github.com/user-attachments/assets/3f4e9add-d47c-4e00-879b-e1a5ed880478">

Тут есть очевидная проблема уверенности в том, что сообщение было доставлено адресату. Несмотря на то что в нашей системе присутствует механизм подтверждения сообщений, всегда есть сомнение, что сообщение не дошло до адресата. 
